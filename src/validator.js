...
const validator = {
  isValid: function (cardNumberValue) {
    /* recibiendo parametro desde index */

    const reversingCardNumber = function (cardNumberValue) {
      /* ocupa el valor porque es una funcion interna */
      return [...cardNumberValue].reverse();
    }; /*string to array*/ /*reversed*/

    let cardNumberReversed =
      reversingCardNumber(
        cardNumberValue
      ); /* regresa el valor una vez que se ejecuto reversingCardNumber */
    ...
    function sumDigits(number) {
      let numberToString =
        number.toString(); /*recibir numero y convertilo a un string*/
    ...
    let multiResult = multiplicationPairNumbersArray.map(function (x) {
      return x > 9
        ? sumDigits(x)
        : parseInt(x); /* para convertir los strings a number*/
      /*sumarlos*/
    });
    /* console.log(multiResult)*/

    let sumAllDigits = multiResult.reduce(
  ...
  maskify: function (cardNumberValue) {
    ...
    let maskedArray = maskArray.map(
      ...
      function (x, i) {
        ...
        /* si la posicion es menor que el lenght entonces poner # y siempre le va a restar -4  */
      }
    );
    maskedArray = maskedArray.toString().replace(/,/g, "");
    /*que reemplace todas las uniones en cualquier posicion*/
